...
const game = {
    ...
    definitions: {
        meridians: [
            {name: "丹田初开", desc: "打开丹田气海", cost: 100, position: {x: 0, y: 0}, connectTo: null, bonus: {type: "maxQi", value: 100}},
            {name: "手太阴肺经", desc: "增加气血上限", cost: 200, position: {x: -80, y: -40}, connectTo: "丹田初开", bonus: {type: "maxHp", value: 100}},
            {name: "手阳明大肠经", desc: "提升攻击力", cost: 300, position: {x: -40, y: -80}, connectTo: "丹田初开", bonus: {type: "attack", value: 10}},
            {name: "足阳明胃经", desc: "提升防御力", cost: 300, position: {x: 40, y: -80}, connectTo: "丹田初开", bonus: {type: "defense", value: 8}},
            {name: "足太阴脾经", desc: "增加悟性", cost: 500, position: {x: 80, y: -40}, connectTo: "丹田初开", bonus: {type: "wuxing", value: 5}},
            {name: "手少阴心经", desc: "增加内力上限", cost: 600, position: {x: -80, y: 40}, connectTo: "丹田初开", bonus: {type: "maxQi", value: 200}},
            {name: "手太阳小肠经", desc: "提升攻击力", cost: 800, position: {x: -40, y: 80}, connectTo: "丹田初开", bonus: {type: "attack", value: 15}},
            {name: "足太阳膀胱经", desc: "提升防御力", cost: 800, position: {x: 40, y: 80}, connectTo: "丹田初开", bonus: {type: "defense", value: 12}},
            {name: "足少阴肾经", desc: "气血内力双提升", cost: 1000, position: {x: 80, y: 40}, connectTo: "丹田初开", bonus: {type: "all", value: 5}},
        ],
        skills: {
            basicPunch: {name: "太祖长拳", type: "外功", desc: "基础拳法，初学者必备", baseDamage: 10, damagePerLevel: 2, qiCost: 5, baseCost: 10, costMult: 1.5},
            basicSword: {name: "基础剑法", type: "外功", desc: "入门剑法，简洁实用", baseDamage: 15, damagePerLevel: 3, qiCost: 8, baseCost: 20, costMult: 1.6},
            commonInner: {name: "吐纳法", type: "内功", desc: "基础内功，缓慢恢复内力", baseRecover: 1, recoverPerLevel: 0.5, qiCost: 0, baseCost: 15, costMult: 1.4},
            lightfoot: {name: "踏雪寻踪", type: "轻功", desc: "提升闪避几率", baseDodge: 5, dodgePerLevel: 2, qiCost: 0, baseCost: 12, costMult: 1.5},
        },
        areas: {
            newbieVillage: {name: "新手村 - 野猪林", desc: "村口野猪林，适合练手", minLevel: 1, hpMin: 40, hpMax: 60, atkMin: 3, atkMax: 6, expPerMin: 2, xiuweiPerMin: 2, drops: {herb: 0.4, ironOre: 0.15}},
            greenWolf: {name: "青狼林", desc: "野狼出没，危机四伏", minLevel: 5, hpMin: 100, hpMax: 180, atkMin: 10, atkMax: 20, expPerMin: 5, xiuweiPerMin: 5, drops: {herb: 0.4, ironOre: 0.2, wolfPelt: 0.15}},
        },
        items: {
            herb: {name: "草药", type: "material", desc: "普通草药，可以炼制丹药"},
            ironOre: {name: "铁矿石", type: "material", desc: "可以锻造武器装备"},
            wolfPelt: {name: "狼皮", type: "material", desc: "完整的狼皮，可以制作皮衣"},
            smallPill: {name: "小还丹", type: "consumable", desc: "恢复50点气血", effect: "restoreHp", value: 50},
        }
    },
    ...
    initDefaultData: function() {
        ...
        // 默认装备基础技能
        if (!this.equippedSkills.outer) this.equippedSkills.outer = 'basicPunch';
        if (!this.equippedSkills.inner) this.equippedSkills.inner = 'commonInner';
        if (!this.equippedSkills.light) this.equippedSkills.light = 'lightfoot';
    ...
    // 打通经脉
    unlockMeridian: function(name, cost) {
        if (this.player.xiuwei >= cost && !this.meridians[name]) {
    ...
    renderInventory: function() {
        ...
        html += `
            <div style="margin-top: 20px; padding: 15px; background: rgba(60, 100, 30, 0.5); border-radius: 6px;">
                <h3>🧪 炼丹</h3>
                <p>2 草药 → 1 小还丹 (恢复50气血)</p>
                <button onclick="game.makePill()" ${(this.inventory.herb || 0) >= 2 ? '' : 'disabled'}>炼制小还丹</button>
            </div>
        `;
    ...
    // 炼制小还丹
    makePill: function() {
        if ((this.inventory.herb || 0) >= 2) {
            this.inventory.herb -= 2;
            this.inventory.smallPill = (this.inventory.smallPill || 0) + 1;
            if (this.inventory.herb <= 0) delete this.inventory.herb;
            this.save();
            this.renderAll();
        }
    },
    ...
    renderMeridianList: function() {
        ...
        game.definitions.meridians.forEach(m => {
            ...
            if (m.bonus && m.bonus.type && m.bonus.value) {
                if (m.bonus.type === 'all') {
                    bonusText = `<small>效果：全属性 +${m.bonus.value}</small>`;
                } else {
                    const name = attrMap[m.bonus.type] || m.bonus.type;
                    bonusText = `<small>效果：${name} +${m.bonus.value}</small>`;
                }
            }
            html += `
                <div class="${cls}">
                    ...
                    <button onclick="game.unlockMeridian('${m.name}', ${m.cost})" ${disabled ? 'disabled' : ''}>
                        ${unlocked ? '已打通' : `打通 (${m.cost}修为)`}
                    </button>
                </div>
            `;
        });
